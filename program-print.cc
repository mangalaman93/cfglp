...
void basic_Block::print_BB(ostream * bb_fp)
{
    ...
    *bb_fp << " Basic Block Number " << bb_num <<".";
    if(true_goto != NULL && false_goto != NULL)
        *bb_fp << "  Successors: True Edge " << true_goto->get_BB_Num() << ", False Edge "<< false_goto->get_BB_Num()<<"\n"; 
    else if(true_goto != NULL)
        *bb_fp << "  Successor "<< true_goto->get_BB_Num() << "\n";
    else *bb_fp << "  No Successor \n";
...
void basic_Block::print_Assembly(ostream * op_fp)
{
    list <icode_Stmt_Ptr>::iterator i;
    *op_fp << "\nlabel" << bb_num << ":\n";
    for (i = icode_list.begin(); i != icode_list.end(); i++)
...
void procedure::print_Epilogue(ostream * op_fp)
{
    ...
    epilogue << "\tlw $fp, 0($sp)  \n\tjr        $31\t\t# Jump back to operating system.\n# Epilogue Ends\n\n";
