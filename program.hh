...
class basic_Block
{
    ...
    icode_Stmt_List icode_list;
    bb_Ptr true_goto, false_goto;
    int lineno;

  public:
    basic_Block(int num, ast_List_Ptr asl, int line);
    ~basic_Block() {}
    ...
    void set_Ast_List(ast_List_Ptr l_ptr);
    int get_Line_Number ();

    bb_Ptr get_True_Successor();
    bb_Ptr get_False_Successor();

    void setSuccessor(bb_Ptr true_S, bb_Ptr false_S);

    void print_BB(ostream *p);
    void print_Icode(ostream * ic_fp);
    void print_Assembly(ostream * op_fp);
